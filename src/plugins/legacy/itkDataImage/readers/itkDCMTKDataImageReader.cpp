...
void itkDCMTKDataImageReader::dicomDictionnaryToKeys()
{
    DcmDataDictionary dico(true, false);
    int x = 0;
    for (auto it = dico.normalBegin(); it != dico.normalEnd(); ++it)
    {

        QString nameTagDcm = (*it)->getTagName();

        auto dcmGroup = (*it)->getGroup();
        auto dcmElem  = (*it)->getElement();

        auto tagGroup = QString("%1").arg(dcmGroup, 4, 16, QLatin1Char('0'));
        auto tagElem  = QString("%1").arg(dcmElem,  4, 16, QLatin1Char('0'));
        x++;
        medMetaDataKeys::addKeyByTagToChapter(tagGroup + ':' + tagElem, "", nameTagDcm, "dicom");
    }
    qDebug() << x;
}
