...
void medViewParameterGroup::updatePool()
{
    ...
    for(medAbstractView *view : d->impactedViews)
    {
        QList<medAbstractParameter*>  params = QList< medAbstractParameter*>(); //TODO FLO view->linkableParameters();
        for(medAbstractParameter* param : params)
        {
            if(this->parametersToLink().contains(param->id()))
            {
                d->pool->append(param);
            }
        }
